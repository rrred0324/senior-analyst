...
> - **治理评级**：上市公司分析必须包含六维度A+~F治理评级→治理折价映射
> - **数据置信度L1-L6**：一手文档(L1,95%+)到训练数据(L6,30%)，与证据等级交叉标注
> - **驱动因子分解法**：概率赋值禁止无锚主观判断，必须分解关键条件独立概率后联合计算
> - **双轨修复**：分析缺口必须同时补内容+修方法论，不可只补报告
> - **IPO Playbook**：新增七步法IPO分析（S-1提取→估值底线→溢价量化→IRR→治理→观察窗口→跟踪框架）
>
> **v2.1.0 升级要点（2026-05）**
...
> - **L2 轻量 Council**：5 项快速审查 + 1-2 个 Bull/Bear，融入关键判断旁
> - **L3 完整 Council**：7 类谬误全覆盖 + 全覆盖 Bull/Bear + Chairman 仲裁，独立章节
> - **L3 触发拓宽**：行业建模/商业模式/战略/投资/竞对场景默认推荐 L3
> - **L2→L3 升级追问**：L2 Council 发现重要分歧时主动建议升级
>
> **v1.8.0 升级要点**
...
L2/L3 分析自动触发的对抗性审查和多视角分析，借鉴 Karpathy llm-council 三阶段架构：
...
**与 v1.8 置信度交互**：数据置信度 + Council 分歧 = 双重验证维度
...
| 深度级别 | 触发信号 | 响应时间 | 说明 |
|---------|---------|---------|------|
| **L1 框架速览** | "入门""框架""指标""概述" | <5 秒 | 直接输出行业速查卡，不触发网络请求，不触发 Council |
| **L2 定量分析** | 包含具体公司名/股票代码 | 10-20 秒 | MCP 必查项并行查询 + 轻量 Council（5 项审查 + 1-2 Bull/Bear） |
| **L3 深度报告** | "深度""尽调""全面" 或行业建模/商业模式/战略/投资/竞对 | 30-60 秒 | 完整查询序列 + 完整 Council（7 类谬误 + 全覆盖 Bull/Bear + Chairman） |
